...
// Convert graph data to React Flow format with safety checks
const convertToReactFlow = (graphData: GraphData | null | undefined) => {
  ...
  const reactFlowNodes: Node[] = positionedNodes.map((node) => ({
    ...
    type: 'custom',
    style: {
      backgroundColor: node?.color || '#f3f4f6',
...
export const GraphVisualization: React.FC = () => {
  ...
  // Handle search results highlighting with safety checks
  useEffect(() => {
    if (searchResults && Array.isArray(searchResults) && searchQuery) {
      setNodes((prevNodes) =>
        prevNodes.map((node) => {
          const isHighlighted = searchResults.some(result => result?.id && result.id === node?.id);
          return {
            ...node,
            data: {
              ...node.data,
      ...
      setEdges((prevEdges) =>
        prevEdges.map((edge) => {
          const isHighlighted = searchResults.some(result => 
            result?.id && (result.id === edge?.source || result.id === edge?.target)
          );
          return {
            ...edge,
  ...
  const onNodeClick = useCallback((event: React.MouseEvent, node: Node) => {
    try {
      ...
      // Safe error handling for node click
