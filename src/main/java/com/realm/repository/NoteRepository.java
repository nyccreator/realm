...
public interface NoteRepository extends Neo4jRepository<Note, String> {
     ...
     */
    @Query("MATCH (n:Note)-[:CREATED_BY]->(u:User {id: $userId}) RETURN n ORDER BY n.updatedAt DESC")
     ...
     */
    @Query(value = "MATCH (n:Note)-[:CREATED_BY]->(u:User {id: $userId}) RETURN n ORDER BY n.updatedAt DESC",
           countQuery = "MATCH (n:Note)-[:CREATED_BY]->(u:User {id: $userId}) RETURN count(n)")
     ...
     */
    @Query("MATCH (n:Note)-[:CREATED_BY]->(u:User {id: $userId}) WHERE n.status = $status RETURN n ORDER BY n.updatedAt DESC")
     ...
     */
    @Query("MATCH (n:Note)-[:CREATED_BY]->(u:User {id: $userId}) WHERE n.isFavorite = true RETURN n ORDER BY n.updatedAt DESC")
    List<Note> findFavoritesByUserId(@Param("userId") String userId);
    
    /**
     * Find notes by category for a user
     */
    @Query("MATCH (n:Note)-[:CREATED_BY]->(u:User {id: $userId}) WHERE n.category = $category RETURN n ORDER BY n.updatedAt DESC")
     ...
     */
    @Query("MATCH (n:Note)-[:CREATED_BY]->(u:User {id: $userId}) WHERE $tag IN n.tags RETURN n ORDER BY n.updatedAt DESC")
    ...
    @Query("""
        ...
        MATCH (node)-[:CREATED_BY]->(u:User {id: $userId})
     ...
     */
    @Query("MATCH (n:Note)-[:CREATED_BY]->(u:User {id: $userId}) WHERE n.updatedAt >= $since RETURN n ORDER BY n.updatedAt DESC")
     ...
     */
    @Query("""
        MATCH (source:Note)-[r:REFERENCES]->(target:Note {id: $noteId})
        MATCH (source)-[:CREATED_BY]->(u:User {id: $userId})
     ...
     */
    @Query("""
        MATCH (source:Note {id: $noteId})-[r:REFERENCES]->(target:Note)
        MATCH (source)-[:CREATED_BY]->(u:User {id: $userId})
     ...
     */
    @Query("""
        MATCH (start:Note {id: $noteId})-[:CREATED_BY]->(u:User {id: $userId})
     ...
     */
    @Query("""
        MATCH (n:Note)-[:CREATED_BY]->(u:User {id: $userId})
     ...
     */
    @Query("MATCH (n:Note {id: $noteId}) SET n.lastAccessedAt = $timestamp RETURN n")
